...
class MusicMetadataParser {
  ...
  static extractDOMContext() {
    ...
    // 5.1 检查标签栏中的 BGM 发现标签
    const biliDiscoveryTag = document.querySelector(
      '.bgm-tag, .bgm-link, .tag-link[title*="发现《"], [class*="bgm-link"], [class*="bgm-tag"], a[href*="music-detail"], a[href*="music.bilibili.com"], .tag-link[href*="music_id"]'
    );
    if (biliDiscoveryTag) {
      const href =
        biliDiscoveryTag.getAttribute('href') || biliDiscoveryTag.getAttribute('data-href') || '';
      const matchId = href.match(/music_id=([A-Za-z0-9_]+)/i);
      if (matchId) {
        musicId = matchId[1];
      }
      const titleAttr = biliDiscoveryTag.getAttribute('title') || '';
      const text = (titleAttr || biliDiscoveryTag.textContent || '').trim();
      const matchTitle = text.match(/发现[《<](.+?)[》>]/);
      if (matchTitle) {
        discoveryTitle = matchTitle[1].trim();
      }
    }

    // 5.2 检查 Bilibili 页面内的音乐详情弹窗卡片 (._PcDetailInfo_)
    const biliDetailInfo = document.querySelector(
      '[class*="_PcDetailInfo_"], [class*="PcDetailInfo"], ._PcDetailInfo_85135_6'
    );
    if (biliDetailInfo) {
      const titleEl = biliDetailInfo.querySelector('.title, [class*="title"]');
      const singerEl = biliDetailInfo.querySelector('.singer, .noMidSinger, [class*="singer"]');
      const coverEl = biliDetailInfo.querySelector('img.cover, .left img, [class*="cover"]');
      if (titleEl && !discoveryTitle) {
        discoveryTitle = (titleEl.textContent || '').replace(/[《》]/g, '').trim();
      }
      if (singerEl) {
        biliArtist = (singerEl.textContent || '').trim();
      }
      if (coverEl) {
        const rawSrc = coverEl.getAttribute('src') || coverEl.src || '';
        if (rawSrc) {
          biliCover = rawSrc.replace(/@.*$/, '').trim();
        }
      }
    }
    ...
    // 6.0 若处于 YouTube 页面且说明栏处于折叠态，静默触发展开以唤醒 Polymer 音乐卡片与图片懒加载
    if (
    ...
    // 6.1 优先提取 YouTube 最新版卡片式音乐模型 (yt-video-attribute-view-model)
    ...
    if (ytViewModel) {
      ...
      const coverEl = ytViewModel.querySelector(
        '.ytVideoAttributeViewModelHeroImage, .ytVideoAttributeViewModelHeroSection img, img'
      );
  ...
  static parse(rawTitle, domContext = null) {
    ...
    const activeCover = ytCover || biliCover || '';

    // 优先：若 YouTube Content ID 结构化信息完整存在，直接采用
    if (ytSong) {
      return {
    ...
    if (partTitle && this.stripNoise(partTitle) !== this.stripNoise(mainTitle || raw)) {
      ...
      const queryTitle = discoveryTitle || partParsed.title || this.stripNoise(partTitle);
      const queryArtist = biliArtist || partParsed.artist || mainParsed.artist || authorClean || '';
    ...
    const queryTitle = discoveryTitle || trackParsed.title || cleanFallback;
...
class MusicMetadataService {
  ...
  static async fetchMetadata(parsedInfo, customHost = '') {
    ...
    // 1. 第一优先级：优先检索 LrcApi (Apple Music / 网易云超清原盘封面 3000x3000px + 同步歌词)
    ...
    // 1.3 降级为单歌名模糊检索（仅在未提供明确 artist 时）
    if (!result && !queryArtist) {
    ...
    // 2. 第二优先级：若 LrcApi 未匹配，但有 Bilibili 官方 musicId，调取 Bilibili 官方版权库
    if (musicId) {
      result = await this._queryBiliMusicDetail(musicId);
      if (result) {
        this._cache.set(cacheKey, result);
        return result;
      }
    ...
    // 3. 第三优先级：若在线接口均未搜到，但 DOM 中已提取到了官方结构化封面与元数据（如 Bilibili 弹窗/YouTube卡片），直接采用
  ...
  static async _queryBiliMusicDetail(musicId) {
    ...
    try {
      ...
      if (data && data.code === 0 && data.data) {
        const d = data.data;
        const title = (d.music_title || '').trim();
        const artist = (
          d.origin_artist ||
          d.artists_list?.map(a => a.name.trim()).join(', ') ||
          ''
        ).trim();
        const album = (d.album || title).trim();
        const cover = (d.mv_cover || '').trim();
