...
class MusicMetadataParser {
  ...
  static extractDOMContext() {
    ...
    // 5. 查找 Bilibili 官方发现音乐标签 / 歌曲识别卡 (如: 发现《Home (feat. Hikaru Utada)》)
    let musicId = '';
    let discoveryTitle = '';
    const biliDiscoveryTag = document.querySelector(
      'a[href*="music-detail"], a[href*="music.bilibili.com"], .tag-link[href*="music_id"]'
    );
    if (biliDiscoveryTag) {
      const href = biliDiscoveryTag.getAttribute('href') || '';
      const matchId = href.match(/music_id=([A-Za-z0-9_]+)/i);
      if (matchId) {
        musicId = matchId[1];
      }
      const text = (biliDiscoveryTag.textContent || '').trim();
    ...
    if (ytViewModel) {
      ...
      const coverEl = ytViewModel.querySelector('.ytVideoAttributeViewModelHeroImage, img');
      ...
      if (coverEl) ytCover = (coverEl.getAttribute('src') || coverEl.src || '').trim();
    }
...
class MusicMetadataService {
  ...
  static async fetchMetadata(parsedInfo, customHost = '') {
    ...
    if (!result && isAlbumCollection && queryArtist && queryAlbum) {
    ...
    // 4. 若在线接口未搜到，但 DOM 中已提取到了官方结构化封面与元数据，直接采用
