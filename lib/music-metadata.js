...
class MusicMetadataParser {
  ...
  static stripNoise(text) {
    ...
    return text
      ...
      .replace(/(?:^|\s)\d{1,2}:\d{2}(?::\d{2})?(?:$|\s)/g, ' ')
      .replace(/^(?:P\d+|EP\d+|Track\s*\d+|\d{1,3}[.\-_\s、]+)\s*/i, '')
      .replace(
        /【(?:Hi-?Res|Hi_?Res|4K|1080P|720P|60帧|60FPS|无损|超清|高清|全专|全专辑|整轨(?:专辑)?|分轨|母带|黑胶|FLAC|DSD|WAV|24bit|96kHz|192kHz|歌词版|官方|自制|翻唱|Cover|MV|LIVE|现场|中字|中英|双语|纯享|无杂音|完整版|动态歌词|修复|杜比|Dolby|首发|新歌|Remix|精修|原声|伴奏|字幕).*?】/gi,
        ' '
      )
      .replace(
        /\[(?:Hi-?Res|Hi_?Res|4K|1080P|720P|60FPS|MV|Official|Music Video|Audio|Live|Lyrics|Remix|HD|HQ|Special|Full|Cover|Full Album|Full Audio Album|整轨|FLAC|DSD|WAV).*?\]/gi,
        ' '
      )
      .replace(
      ...
      .replace(
        /(?:全专试听|全专辑试听|全专歌词版|全专辑歌词版|全专无损|全专|全专辑|原声专辑合集|原声合集|专辑合集|全专合集|全专辑合集|原声专辑|官方音频专辑|完整专辑|合集试听|全碟试听|全碟|整轨专辑|整轨试听|整轨纯享|整轨|分轨|无损纯享|纯享整轨专辑|纯享整轨|纯享|无损音质|高音质|无损|官方MV|官方超清|(?:\b|^)MV(?:\b|$)|(?:\b|^)OST(?:\b|$))/gi,
        ' '
      )
      .replace(/\s+/g, ' ')
      .trim();
  ...
  static cleanArtist(text) {
    ...
    return text
      ...
      .replace(/[（(][^）)]*?(?:乐队|歌手|时代|乐团|Boy|Group|Band|Official)[^）)]*?[)）]/gi, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  }
  ...
  static parseAlbumCollection(rawText, duration = 0) {
    ...
    const isCollection =
      collectionKeywordRegex.test(rawText) ||
