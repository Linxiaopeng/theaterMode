...
(function (global) {
  class BlurBackgroundController {
    ...
    /**
     * 设置自定义静态封面图片源（例如在线高清专辑封面）
     */
    setCustomImageSource(imgElement) {
     ...
     */
    update() {
      const isCustom = !!this.customImage;
      const source = this.customImage || this.video;
      if (!source) return;
      ...
      if (isCustom) {
        ...
        if (source.paused && this.hasRenderedFrame) return;
        if (this.options.isStatic && this.hasRenderedFrame) return;
        ...
        try {
          ...
          if (imgData && imgData.data && imgData.data.length > 0) {
            ...
            if (hash && hash !== this.lastHash) {
              this.lastHash = hash;
              const decodedPixels = BlurHash.decode(
                hash,
                this.options.decodeWidth,
                this.options.decodeHeight
              );
              if (decodedPixels) {
      ...
      if (this.mode === 'lowfreq' || !this.options.enableBlurHash) {
        try {
          ...
          this.ctx.drawImage(source, 0, 0, this.options.lowFreqWidth, this.options.lowFreqHeight);
          this.hasRenderedFrame = true;
          return;
        } catch (e) {
          this.mode = 'clone';
      ...
      if (this.mode === 'clone') {
        if (!this.clonedVideo && this.canvas.parentNode) {
          ...
          this.clonedVideo.style.cssText =
    ...
    bindEvents() {
      ...
      this.timer = setInterval(() => {
        if (this.video && !this.video.ended && !this.video.paused) {
          this.update();
        }
